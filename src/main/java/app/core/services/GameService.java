...
public class GameService {
			...
			if (game.getActivePlayers().size() == 2) {
				...
				game.getPlayers().get(game.getNextPlayer(game.getDealer()))
						.setWallet(game.getPlayers().get(game.getNextPlayer(game.getDealer())).getWallet()
								- game.getBigBlindBet());
				game.getPlayers().get(game.getDealer()).setLastAct(PlayEnum.RAISE);
				game.getPlayers().get(game.getDealer()).setLastAmount(game.getBigBlindBet());
	...
	public GameData pingGame(String token) throws HoldemException {
		try {
			...
			if (game.getStatus()!= StatusEnum.WAITING) {
				if (player.get() == getCurrentPlayer(game)
	...
	public GameData play(String token, PlayEnum play, int amount) throws HoldemException {
		try {
			...
			if (player.getId() != game.getActivePlayers().get(game.getPlayerturn())) {
	...
	private void printStatusDebug(String note, Game game, boolean lastTurn) {
		System.out.print(note);
		System.out.println(" ----------------------");
		System.out.print("LAST RAISED: ");
		System.out.println(game.getLastRaised());
